package io.trtc.tuikit.atomicx.albumpicker.ui.component.videoview;
...
public class VideoGestureScaleAttacher {
    ...
    public static void attach(VideoView view) {
        ...
        attacher.internalScaleListener = new OnScaleListener() {
            ...
            public void onFling(float startX, float startY, float velocityX, float velocityY) {
                ...
                attacher.currentFlingRunnable.fling(
                    attacher.getViewWidth(), attacher.getViewHeight(), (int) velocityX, (int) velocityY);
        ...
        view.setOnTouchListener(new View.OnTouchListener() {
            ...
            private void onActionCancel() {
                if (attacher.getScale() < attacher.minScale) {
                    ...
                    view.post(attacher.new AnimatedZoomRunnable(
                        attacher.getScale(), attacher.minScale, rect.centerX(), rect.centerY()));
                } else if (attacher.getScale() > attacher.maxScale) {
                    RectF rect = attacher.getDisplayRect();
                    view.post(attacher.new AnimatedZoomRunnable(
                        attacher.getScale(), attacher.maxScale, rect.centerX(), rect.centerY()));
                }
            }
        });
    }
    ...
    public float getScale() {
        return (float) Math.sqrt((Math.pow(getValue(transferMatrix, Matrix.MSCALE_X), 2)
                                     + Math.pow(getValue(transferMatrix, Matrix.MSCALE_Y), 2))
            / 2);
    }
